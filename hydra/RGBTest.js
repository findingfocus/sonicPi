...
src(s0)
  .layer(
    src(s0)
      ...
      .diff(
        src(s0).g()
          .scale(()=>1 - 0.01 * Math.sin(time))
          .rotate(()=>-0.01 * Math.sin(time)) // Opposing rotation for depth effect
          .scrollX(()=>0.01 * Math.cos(time * 0.3)) // Offset wave effect for variation
      ...
      .diff(
        src(s0).b()
          .scale(()=>1 + 0.03 * Math.cos(time))
          .rotate(()=>0.01 * Math.cos(time)) // Complementary rotation for blue
          .scrollX(()=>0.01 * Math.sin(time * 0.5))
          .scrollY(()=>0.01 * Math.cos(time * 0.5))
          .color(0, 0, 1)
          .brightness(0.5)
          .contrast(1.5)
      )
